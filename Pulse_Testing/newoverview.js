...
describe("Vantage Circle Automation Tests", function () {
  ...
  it("Logs into the application", async function () {
    try {
      ...
      const email = "kongkona.das@vantagecircle.com";
  ...
  it("Accesses the dashboard", async function () {
    try {
        ...
        const profileClick = await waitForElement("//img[@alt='Profile Image']", 30000); // Increased timeout
        await profileClick.click();

        const dashboardButton = await waitForElement("//span[normalize-space()='Admin Dashboard']", 10000); // Increased timeout
 ...
 it("Clicks on product's menu dropdown - select Pulse", async function () {
            ...
            it("Clicks on survey view more", async function () {
              ...
              /*it("Clicks on survey overview dropdown - select any survey", async function () {
                ...
                it("Clicks the Insights - Question Score", async function () {
                  try {
                    await driver.wait(async () => (await driver.getAllWindowHandles()).length === 2,20000);
            
                        let InsightsQuestion = await driver.findElement(By.xpath('//*[@id="router"]/vantage-circle-dashboard-pulse/vantage-circle-dashboard-insights/div/vantage-circle-dashboard-insights-sub-header/div/div[5]',20000));
                        await driver.sleep(1000); // ensure animations or transitions complete
                        await InsightsQuestion.click();
                        await driver.sleep(3000); // wait for response
                ...
                it("Clicks Configuration", async function () {
                  try {
                    //await driver.wait(async () => (await driver.getAllWindowHandles()).length === 2,20000);
            
                        let clickConfig = await driver.findElement(By.xpath("/html/body/vantage-circle-dashboard-root/vantage-circle-dashboard-portal/div/div/div[1]/vantage-circle-dashboard-sidebar/div/div/div[2]/div[4]/div[1]/p",20000));
                        await driver.sleep(1000); // ensure animations or transitions complete
                        await clickConfig.click();
                        await driver.sleep(4000); // wait for response
            
                    } catch (error) {
                        console.error("Error when clicking the config button:", error);
                        await driver.takeScreenshot().then(function(image) {
                            require('fs').writeFileSync('click-config-button-failure.png', image, 'base64');
                        });
                        throw error;
                    }
                });

                it("Clicks Configuration -preview an existing email", async function () {
                  try {
                    //await driver.wait(async () => (await driver.getAllWindowHandles()).length === 2,20000);
            
                        let ConfigPreview = await driver.findElement(By.xpath("(//button[contains(text(),'Preview')])[1]",20000));
                        await driver.sleep(1000); // ensure animations or transitions complete
                        await ConfigPreview.click();
                        await driver.sleep(4000); // wait for response
                ...
                it("Clicks Configuration -Manage Survey", async function () {
                  try {
                    //await driver.wait(async () => (await driver.getAllWindowHandles()).length === 2,20000);
            
                        let ConfigSurvey = await driver.findElement(By.xpath("(//div[@class='text-gun-powder hover:text-purple-500 py-3 border-b-2 border-purple-500 text-purple-500'])[1]",20000));
                        await driver.sleep(1000); // ensure animations or transitions complete
                        await ConfigSurvey.click();
                        await driver.sleep(4000); // wait for response
                ...
                it("Clicks Configuration -Manage Questions", async function () {
                  try {
                    //await driver.wait(async () => (await driver.getAllWindowHandles()).length === 2,20000);
            
                        let ConfigQuestion = await driver.findElement(By.xpath("(//div[normalize-space()='Manage Questions'])[1]",20000));
                        await driver.sleep(1000); // ensure animations or transitions complete
                ...
                it("Clicks Learning", async function () {
                  try {
                    await driver.wait(async () => (await driver.getAllWindowHandles()).length === 2,20000);
            
                        let clickLearn = await driver.findElement(By.xpath("(//p[@class='menu-title flex w-full items-center justify-between text-gun-powder group-hover:text-lucky-point group-hover:font-medium'])[5]",20000));
                        await driver.sleep(1000); // ensure animations or transitions complete
                        await clickLearn.click();
                        await driver.sleep(3000); // wait for response
            
                    } catch (error) {
                        console.error("Error when clicking the Learning button:", error);
                        await driver.takeScreenshot().then(function(image) {
                            require('fs').writeFileSync('click-learn-button-failure.png', image, 'base64');
                        });
                        throw error;
                    }
                });

                it("Clicks Learning - Read more", async function () {
                  try {
                    await driver.wait(async () => (await driver.getAllWindowHandles()).length === 2,20000);
            
                        let clickLearnMore = await driver.findElement(By.xpath("(//a[@href='https://vantagecircle.ghost.io/best-payroll-software/'])[1]",20000));
                        await driver.sleep(1000); // ensure animations or transitions complete
                        await clickLearnMore.click();
                        await driver.sleep(3000); // wait for response
                ...
                //swtiching to previous window
                it("Accesses the dashboard - switch to previous window", async function () {

                  try {
                    await driver.wait(
                            async () => (await driver.getAllWindowHandles()).length === 1,20000);
                          const windowHandles = await driver.getAllWindowHandles();
                          await driver.switchTo().window(windowHandles[1]);
                          let dashboardTitle = await driver.getTitle();
                          assert.strictEqual(dashboardTitle, "Vantage Circle | Dashboard");
            ...
            it("Clicks the existing survey", async function () {
              try {
                await driver.wait(async () => (await driver.getAllWindowHandles()).length === 2,20000);
        
                    let select_survey = await driver.findElement(By.xpath("(//button[contains(text(),'Use Template')])[3]",20000));
                    await driver.sleep(1000); // ensure animations or transitions complete
                    await select_survey.click();
                    await driver.sleep(8000); // wait for response
