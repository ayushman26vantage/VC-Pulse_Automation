...
it("Check if Access your Dashboard button is enabled", async function () {
  try {
   // await driver.wait(async () => (await driver.getAllWindowHandles()).length === 2, 30000);

    let dash_drop = await driver.wait(
      until.elementLocated(By.xpath("(//button[normalize-space()='Access your dashboard'])[1]")),
      20000
    );

    let isEnabled = await dash_drop.isEnabled();

    // Assertion
    assert.strictEqual(isEnabled, true, "'Access your Dashboard' button should be enabled");
    console.log("✅ Assertion passed: 'Access your Dashboard' button is enabled.");

    console.log("🕒 Waiting for 3 seconds after assertion...");
    await driver.sleep(3000);

  } catch (error) {
    // await captureScreenshot("dashboard-button-disabled");
    throw error;
  }
});


it("Clicks on Access your Dashboard", async function () {
  try {
    //await driver.wait(async () => (await driver.getAllWindowHandles()).length === 2, 30000);
...
it("Clicks the Create button", async function () {
      try {
        await driver.wait(async () => (await driver.getAllWindowHandles()).length === 2,20000);
            ...
            let createButton = await driver.findElement(By.xpath("(//button[contains(text(),'Use Template')])[3]",20000));
